...
**The essay's later additions** extend two of those five sections further and add two wholly new sections after them: system-design philosophy also frames business-driven development explicitly — every design decision is checked against whether it drives revenue, reduces risk, saves time, or streamlines a process, illustrated by a named "AI Growth & Adoption" idea (quoting Thomas directly: a system that scales to a million users is pointless if its actual users haven't adopted it yet, so adoption is the metric that comes before throughput, not after); the agent definition also covers prompt engineering — a default template of objective, steps, at least one explicit constraint, and a known example where one exists, and when to reach for zero-shot prompting (intentional ambiguity, for agentic creativity and flexibility) versus few-shot, chain-of-thought, or tree-of-thought prompting (for deterministic, workflow-shaped tasks); a sixth section, "unintended consequences," covers two real interview anecdotes — an English-to-Chinese/Japanese translation prompt where the target dialect itself (e.g. Kyoto's Kansai-ben vs. Tokyo's standard Japanese) is a bias decision, and a two-way English/Portuguese translation prompt where launching targeted only at Brazil carries geopolitical and PR weight — making the point that a system's design may not need to change, but an engineer's awareness of the real-world business and geopolitical consequences around that design does; and a seventh section, "effective communication," covers audience-first communication — chronological (background, scope, reasons, then solution with a live demo) as the default for a lay or mixed audience, reverse-chronological (demonstration first, questions after) for a technical one, jargon and humor calibrated the same way, and "shared experiences" as a way to build interviewer rapport by grounding questions in real, relatable background. A final, unlabeled block closes the essay — explicitly not one of the mental models itself, but the synthesis of all of them: none are meant to be memorized as a checklist, they're combined live, navigating a problem both horizontally (end-to-end) and vertically (abstraction and time) while applying the business-driven-development rubric and communicating that navigation out loud, collaboratively, as it happens.
...
### 9. THE GLOSSARY (/glossary)
/glossary is a ~200-term, first-principles reference spanning seven categories in two tracks: Foundations (GPU & CUDA Compute, Software Engineering & System Design — independent of each other, everything else depends on one or both) and the ML Lifecycle, in dependency order (Data Engineering → Data Science & ML Modeling / AI-ML Engineering in parallel → MLOps & Infrastructure → Evaluation, Monitoring & Observability, kept as its own category rather than folded into MLOps or System Design). Its signature element is a "stack diagram" — a silicon-to-product cross-section where each category's band height is computed live from its actual term count, not hand-tuned, so GPU/CUDA and System Design sit as two visually distinct bottom strata rather than one fused "Foundations" block.

**The voice is deliberate**: every definition is 1-3 sentences, mechanism-first, and explicitly omits examples — the standing model for that voice, written by Thomas himself, is "an encoder is to programmatically represent inputs and a decoder is to take the output of an encoder as inputs, to output another representative form." Terms are typed as mechanism (has an input→output transformation), constraint (a measured/agreed bound, e.g. SLI/SLO/SLA/error budget — each defined exactly once, under Evaluation & Observability, never redefined elsewhere), or structure (an organizational/architectural relationship) — never phrased with a worked example or a vendor/product name standing in for the general concept.

**Navigation**: a sticky sidebar (native "On this page" disclosure on mobile) groups categories by track with live term counts, plus an A-Z jump rail that lands on the alphabetically-first term for a given letter; a plain client-side search filters by term, definition, and category label with no debounce or network round-trip, since the ~200 entries are fully static.

If asked "what does X mean," "can you define X," "what's the difference between X and Y" for any software/data/AI/ML term, or generally about this portfolio's own terminology, point the user at /glossary rather than defining the term from memory in the chat itself — it is the durable, cross-linked source of truth for that class of question, and its content, unlike this chat's own knowledge, is grounded directly in the repository's own study notes and source code.
`;
