...
/**
 * Below `lg:`, SystemDesignPrepSidebar disappears entirely and this native
 * <details> disclosure takes its place — zero JS, same collapsed-by-default
 * pattern already used by ChangelogMobileToc / the FAQ accordion.
 */
const SystemDesignPrepMobileToc = () => (
  <details className="group lg:hidden card-base p-0 overflow-hidden mb-10">
    ...
    <div className="px-4 pb-4 space-y-4 border-t border-zinc-100 dark:border-zinc-800 pt-4">
      ...
      <div className="space-y-1">
        <span className="block px-3 text-micro text-zinc-400 mb-1">The Questions</span>
        {QUESTIONS_TOC_ITEMS.map((item) => (
          <a key={item.id} href={item.href} className={LINK_CLASS}>
            {item.label}
          </a>
        ))}
...
export default SystemDesignPrepMobileToc;
